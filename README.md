...
AI/Agent 不沿旧 Python LangGraph/LiteLLM/SymPy/OCR 工作流迁移。Go 侧已接入 Eino Tutor、Portrait、Diagnostician、Math Solver、Question Parser、Question Generator Agent 与后台 LLM/Agent 配置闭环：管理员可通过 `/api/v1/admin/ai-config/*` 管理 provider、model 和 Agent 配置；`/api/v1/session/{id}/chat`、`/api/v1/portrait/generate`、`/api/v1/exercise/submit`、`/api/v1/questions/ai-parse` 和学生专用 `/api/v1/exercise/generate` 分别读取对应 Agent 配置。自主出题未配置或输出非法时返回 `503 AI_GENERATION_UNAVAILABLE` 且不落库；纯图片答案返回 `501 OCR_UNAVAILABLE` 且不更新学习状态。OCR 和更完整的通用数学求解仍需继续在 P6 新工作流中实现。
...
1. **非 AI 能力先稳定**
   ...
   - Eino Tutor/Portrait/Diagnostician/Math Solver/Question Parser/Question Generator Agent 与后台配置闭环是 Go 侧新架构的首批能力，不代表完整 AI 工作流已经完成。
...
| **AI 工作流** | P6 进行中；Eino ADK Tutor/Portrait/Diagnostician/Math Solver/Question Parser/Question Generator Agent 已接入，OCR 仍待实现且图片-only 答案 fail-closed | Go 后端 |
| **LLM 集成** | Tutor/Portrait/Diagnostician/Math Solver/Question Parser/Question Generator Agent 使用 OpenAI-compatible ChatModel；provider/model/Agent 配置、测试和模型拉取接口已接入 | P6 进行中 |
...
| Agent 配置选择 | 首轮可用 | 支持 `math_solver`、`tutor`、`diagnostician`、`portrait`、`question_parser`、`question_generator` 六类 Agent 配置持久化；运行时已读取持久化配置 |
...
| 智能刷题 | `/exercise` | “班级题目 / AI 自主练习”双模式；学生无需加入班级即可按知识点和难度调用 `question_generator` 生成严格四选一题，两类题分别显示班级题辅导或 AI 练习教练并复用文本判题、诊断和个人 DKT；OCR 未开放 |
...
| `AdminAIConfigService` | `/admin/ai-config/*` provider/model/Agent 配置、模型拉取、连接测试和六类 Agent 运行时配置读取 |
...
- **AI 行为**: `/api/v1/admin/ai-config/*` 管理 provider/model/Agent 配置；`/api/v1/session/{id}/chat`、`/api/v1/portrait/generate`、文本形式的 `/api/v1/exercise/submit`、`/api/v1/questions/ai-parse` 和 `/api/v1/exercise/generate` 读取对应 Agent 配置；自主出题无可用模型时返回 503 且不落库，其余既有路径按各自契约降级；纯图片答案固定返回 `501 OCR_UNAVAILABLE`
...
- ✅ AI 接口边界：`/admin/ai-config/*` 已支持 provider/model/Agent 配置闭环；`/session/{id}/chat`、`/portrait/generate`、`/exercise/submit`、`/questions/ai-parse` 和 `/exercise/generate` 已具备持久化配置驱动的 Eino Agent 第一片实现
