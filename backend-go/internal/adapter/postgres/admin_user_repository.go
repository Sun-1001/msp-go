...
func (r UserRepository) DeleteUser(ctx context.Context, userID string) (bool, error) {
	...
	err := r.withTx(ctx, func(tx UserRepository) error {
		...
		statements := []string{
			...
			`DELETE FROM public.content_attempts WHERE student_id = $1 OR content_id IN (SELECT id FROM public.contents WHERE owner_teacher_id = $1)`,
			`DELETE FROM public.contents WHERE owner_teacher_id = $1`,
