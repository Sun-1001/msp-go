...
当前生产迁移链由 `0001` 至 `0010` 十个迁移组成。每日一题及其班级自动提醒、公众号专用事件位于已发布的 `0005_daily_question`；`0006` 至 `0008` 交付画像能力；每日题一致性通过 `0009_daily_question_integrity` 向前追加。远端共享迁移账本只到 version 9，因此工作区的四个未发布错题本迁移已按依赖顺序合并为单一 `0010_mistake_review_tasks`。它一次交付错题复习任务、全部提交幂等、聚合归档、复合归属约束、`mastery_weight`、知识点 JSON 校准、系统“未分类”、DKT/画像一致性和全部已提交作答题面冻结。

全新空库第一次 migration runner 应记录版本 `1` 至 `10`；远端 version 9 数据库应只应用 version 10，紧接着重复执行应为 `applied_count=0`。迁移 10 只为每个学生、每道题最新仍未解决的错误回填任务；既有作答保留默认权重 `1`。迁移会扫描作答事实，校准画像和 DKT，建立索引和外键，并补齐全部已提交作答的题面快照，因此应停止应用写入、完成可恢复备份后在维护窗口执行。旧普通作答的原始题目版本无法恢复，只使用迁移时最佳证据稳定化，不据此重算掌握度。新 API 必须在迁移成功后一次性切换，不能与仍写空快照的旧实例长期混跑。

迁移后应确认版本 10 已记录，所有已提交 attempt 的五个必需快照字段非空，知识点均引用真实节点，`mastery_vector` 为数值范围 `0..1` 的对象，复合归属外键、幂等索引和 `ck_content_attempts_review_question_snapshot` 均已生效，且临时 `msp_0011_*`、`msp_0012_*` 函数均不存在。曾完整执行未发布草稿 version 10 至 13 的本地数据库已经具备最终结构，但账本与当前单文件链不兼容；禁止删除 version 10 后重放合并迁移，必须先备份、核对最终结构，再在排他锁内实施仅迁移元数据的收敛。
