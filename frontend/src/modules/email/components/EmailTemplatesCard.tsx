...
export const EmailTemplatesCard: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const response = await adminEmailService.listTemplates();
        if (active) setTemplates(response.items);
      } catch (error) {
        if (active) {
          setFeedback({ type: 'error', message: getApiErrorMessage(error, '加载邮件模板失败') });
        }
      } finally {
  ...
  const openPreview = async (template: EmailTemplate) => {
    ...
    try {
      ...
      setEditorError(getApiErrorMessage(error, '预览邮件模板失败'));
    } finally {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setEditorError(getApiErrorMessage(error, '保存邮件模板失败'));
    } finally {
      setActiveAction(null);
    }
  };
  ...
  const handleRestore = async () => {
    ...
    try {
      ...
      setFeedback({ type: 'error', message: getApiErrorMessage(error, '恢复官方模板失败') });
    } finally {
      setActiveAction(null);
    }
  };
  ...
  return (
    <>
      ...
      <ConfirmDialog
        ...
        title="恢复官方模板"
        message={restoreTarget ? `${restoreTarget.name}（${localeLabel(restoreTarget.locale)}）将恢复为官方内容。` : ''}
        confirmText="恢复模板"
  ...
  >
    {template ? (
      <div className="relative z-10 space-y-5">
        {error ? (
          <div className="flex items-center gap-2 rounded-md border border-red-200 bg-red-50 p-3 text-sm text-red-700 dark:border-red-800 dark:bg-red-900/20 dark:text-red-300">
